 ...
 * build-registry.mjs
 *
 * Reads all component source files from packages/registry/src/components/ui/
 * and generates JSON manifest files in public/r/<component>.json following
 * the shadcn registry format.
 *
...
function buildRegistry() {
  ...
  // Check that registry source exists
  if (!fs.existsSync(REGISTRY_SRC)) {
    console.error(`Registry source directory not found: ${REGISTRY_SRC}`);
    process.exit(1);
  }

  // Read all .tsx files from the registry source
  const files = fs.readdirSync(REGISTRY_SRC).filter((f) => f.endsWith('.tsx'));

  console.log(`Found ${files.length} component files in registry source.`);

  let count = 0;

  for (const file of files) {
    const componentName = file.replace(/\.tsx$/, '');
    const filePath = path.join(REGISTRY_SRC, file);
  ...
  console.log(`Generated ${count} registry JSON files in ${OUTPUT_DIR}`);
}
