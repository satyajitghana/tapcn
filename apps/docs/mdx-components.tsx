...
import { IconBrandNpm, IconBrandPnpm, IconBrandYarn } from '@tabler/icons-react';
...
// Bun icon — tabler doesn't have one, so we use an inline SVG
...
const packageManagerIcons: Record<string, ReactNode> = {
  npm: <IconBrandNpm className="size-4" />,
  pnpm: <IconBrandPnpm className="size-4" />,
  yarn: <IconBrandYarn className="size-4" />,
  bun: <BunIcon className="size-4" />,
};

function CodeBlockTabsTrigger(
  props: ComponentProps<typeof FumadocsCodeBlockTabsTrigger>,
) {
  const { children, ...rest } = props;
  const text = typeof children === 'string' ? children.toLowerCase() : '';
  const icon = packageManagerIcons[text];

  return (
    <FumadocsCodeBlockTabsTrigger {...rest}>
      {icon}
      {children}
